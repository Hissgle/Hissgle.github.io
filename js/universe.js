/**
 * universe.js — 深邃夜空背景动效
 *
 * 效果：墨蓝到深黑的深邃夜空 + 缓慢漂移呼吸的星云 + 多层星光闪烁（带光晕的大星）
 *       + 随机划过的流星（柔和拖尾、长度衰减、偶发「火流星」爆闪）
 *
 * 特性：
 ...
 *   - 若主题带夜间模式（html.dark），夜间显示、白天自动淡出
 *
 * 用法：无需任何调用，引入即可：
...
(() => {
  ...
  const initUniverse = () => {
    ...
    // ⚠️ 定位样式必须无条件设置，不能用 parentElement === document.body 来判断：
    ...
    // 关键：把画布提到 body 直接子元素层级。
    // 否则它被包在主题那个 <div> 里，而 #body-wrap 等兄弟节点在 DOM 中更靠后，
    // 同层级绘制时会盖在画布之上（负 z-index 也不一定救得回来）。
